...
import { createSpyProvider } from '../../helpers/providers';
import { flushIfReady } from '../../../src';
...
describe('SSR hydration (browser)', () => {
  ...
  //   const spy = createSpyProvider();  // same spy you already use
  ...
  //   await tick(5);
  //   expect(spy._get().eventCalls.length).toBe(0);           // nothing yet
  //   expect(getSSRQueueLength()).toBe(1);                    // still present

  //   facade.grantConsent();
  //   await tick(10);

  //   expect(spy._get().eventCalls.length).toBe(1);           // now replayed
  ...
  it('drops SSR analytics on denied; allows essential when allowEssentialOnDenied', async () => {
    ...
    facade.init({
      autoTrack: false,
      trackLocalhost: true,
      consent: { initialStatus: 'pending', disablePersistence: true },//, allowEssentialOnDenied: true },
    });
    ...
    expect(spy._get().eventCalls.length).toBe(0);
  ...
  //   await tick(10);
  //   expect(spy._get().pageviewCalls.length).toBe(1); // SSR PV only
  // });
});
